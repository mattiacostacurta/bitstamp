...
class TestCsvFile(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data = {
            'high': 42865.80,
            'last': 42047.32,
            'timestamp': 1642502504,
            'bid': 42035.75,
            'vwap': 42146.43,
            'volume': 1077.71980924,
            'low': 41482.63,
            'ask': 42050.46,
            'open': 42230.09}
    ...
    def test_valid_table(self):
        ...
        self.assertEqual(
            self.table1['open'].dtypes,
            self.table2['open'].dtypes)
        self.assertEqual(
            self.table1['volume'].dtypes,
            self.table2['volume'].dtypes)
        self.assertEqual(
            self.table1['last'].dtypes,
            self.table2['last'].dtypes)
    ...
    def test_invalid_table(self):
        ...
        self.assertFalse(
            self.table1['open'].dtypes == self.table3['open'].dtypes)
        self.assertFalse(
            self.table1['volume'].dtypes == self.table3['volume'].dtypes)
        self.assertFalse(
            self.table1['last'].dtypes == self.table3['last'].dtypes)
    ...
    def test_blank_table(self):
        ...
        with respect to the original csv created (CryptoTable)
        '''
        self.assertTrue(os.stat("Example2.csv").st_size !=
                        os.stat("CryptoTable.csv").st_size)
...
class TestReadCsv(unittest.TestCase):
    ...
    def test_valid_read_csv(self):
        ...
        self.assertEqual(
            read_csv('change'), round(
                (42047.32 - 42230.09) / 42047.32 * 100, 2))
...
class TestReadDataframe(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data = {
            'date': [
                1642867355, 1642869224], 'tid': [
                218372056, 218377609], 'amount': [
                0.20473807, 0.05000000], 'type': [
                    1, 0], 'price': [
                        34611.42, 35262.41]}
    ...
    def test_invalid_input(self):
        ...
        '''
        self.assertTrue(read_df(123) is False)
        self.assertTrue(read_df('') is False)
        self.assertTrue(read_df([]) is False)
        self.assertTrue(read_df({}) is False)
